 ...
 * 
 * Design Patterns: - State Pattern: Maneja diferentes estados del juego -
 * Observer Pattern: Notifica cambios de estado - Command Pattern: Encapsula
 * acciones del juego
 * 
 * SOLID Principles: - Single Responsibility: Maneja únicamente el estado del
 * juego - Open/Closed: Permite extensión de nuevos estados sin modificación -
 * Liskov Substitution: Las implementaciones deben ser intercambiables
 * 
 * Conceptos a implementar: - Programación Funcional: Consumer, Supplier,
 * callbacks - Modularización: Separación de estado, lógica y persistencia -
 * OOP: Encapsulación del estado y comportamiento
 */
public abstract class GameState<T extends Position> {

	protected final String gameId;
	// Se quitan los final, porque o sino, no hay forma de cambiar la fecha para un
	// GameState recuperado de la BD.
	...
	/**
	 * Ejecuta un movimiento en el juego. Template Method que define el flujo
	 * general.
	 */
	...
	/**
	 * Verifica si un movimiento puede ser ejecutado. Los estudiantes deben
	 * implementar las reglas específicas.
	 */
	protected abstract boolean canExecuteMove(T position);

	/**
	 * Realiza el movimiento específico en el estado del juego. Aquí se actualiza el
	 * estado interno.
	 */
	protected abstract boolean performMove(T position);

	/**
	 * Actualiza el estado del juego después de un movimiento. Determina si el juego
	 * continúa, termina, etc.
	 */
	protected abstract void updateGameStatus();

	/**
	 * Obtiene la posición actual del gato. Fundamental para la lógica del juego.
	 */
	public abstract T getCatPosition();

	/**
	 * Establece la posición del gato. Usado por los algoritmos de movimiento.
	 */
	public abstract void setCatPosition(T position);

	/**
	 * Verifica si el juego ha terminado. Puede ser por victoria, derrota o empate.
	 */
	public abstract boolean isGameFinished();

	/**
	 * Determina si el jugador ha ganado. Los estudiantes definen las condiciones de
	 * victoria.
	 */
	public abstract boolean hasPlayerWon();

	/**
	 * Calcula la puntuación actual del juego. Los estudiantes pueden implementar
	 * diferentes sistemas de puntuación.
	 */
	public abstract int calculateScore();

	/**
	 * Obtiene una representación serializable del estado. Útil para persistencia y
	 * comunicación con el frontend.
	 */
	public abstract Object getSerializableState();

	/**
	 * Restaura el estado desde una representación serializada. Permite cargar
	 * juegos guardados.
	 */
	...
	public enum GameStatus {
		IN_PROGRESS, PLAYER_WON, PLAYER_LOST, DRAW, PAUSED
	}
}